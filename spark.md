...
The following scripts written in Scala permit very rapid import into MySQL of massive GBIF occurrence csv files contained in a Darwin Core Archive download. Other methods here produce aggregates of these same occurrence data as many smaller csv files for rapid import into relational tables that would otherwise be impractical. The goal here is to produce a unique list of agents as a union of recordedBy and identifiedBy Darwin Core fields while retaining their occurence record memberships. This vastly reduces the number of agents that need to eventually be parsed. Aggregating identifiedBy and recordedBy fields from an occurrence csv file containing 73M records takes 20-30 minutes using 6GB of memory.
