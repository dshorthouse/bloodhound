  ...
  class AgentWorker
    ...
    def perform(file_path)
      CSV.foreach(file_path, :headers => true) do |row|
        ...
        gbifIDs_recordedBy = row["gbifIDs_recordedBy"].tr('[]', '').split(',').map(&:to_i)
        gbifIDs_identifiedBy = row["gbifIDs_identifiedBy"].tr('[]', '').split(',').map(&:to_i)
        agents.each do |a|
          begin
          ...
          if !gbifIDs_recordedBy.empty?
            data = gbifIDs_recordedBy.map{|r| { occurrence_id: r, agent_id: agent.id}}
            OccurrenceRecorder.import data
          end
          if !gbifIDs_identifiedBy.empty?
            data = gbifIDs_identifiedBy.map{|r| { occurrence_id: r, agent_id: agent.id}}
